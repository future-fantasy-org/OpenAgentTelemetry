...
- **全局 `onResponse` 钩子** — 捕获每个 API 响应；只记录写操作（POST/PUT/PATCH/DELETE）+ 错误（≥400）；跳过成功的 GET 和健康检查，控制日志量
- **`audit_logs` 表** — 持久化操作者邮箱/IP、派生动作、method、path、资源类型/ID、项目 ID、状态码、耗时、JSON metadata。按 `created_at DESC`、`project_id`、`action` 建索引
- **`deriveAction` 纯函数** — 将 `{method, path, statusCode}` 映射为语义化动作字符串（如 `auth.login_failed`、`project.create`、`alert_rule.update`、`idor.blocked`）。特殊处理 login/logout/ingestion；IDOR 检测（404 + GET + `/api/`）在资源匹配之前执行
- **`/audit` 页面** — 服务端渲染列表，支持 cursor 分页、动作筛选下拉框、彩色状态徽章、SSE 实时推送新条目

### 实时 SSE + Cursor 分页（M12）

- **EventBus 单例** — 进程级 `EventEmitter`（最多 200 监听器）发射三类强类型事件：`trace:created`、`alert:triggered`、`audit:logged`。`IngestionService` 和 `AlertEvaluator` 在 DB 写入后发射
- **SSE 三路流** — `GET /api/stream/{traces,alert-events,audit-logs}`，`text/event-stream` 响应，30s 心跳，`req.raw.on('close')` 清理，`config: { rateLimit: false }` 绕过全局限流器
- **Cursor 分页** — `Traces` 和 `/api/audit/logs` 返回 `{ data, nextCursor }`，使用 `WHERE created_at < cursor` + `limit + 1` 技巧检测 hasMore。替代 offset 分页，深翻页稳定
- **前端实时更新** — 各页面（Traces、Alerts、Audit）用 `EventSource` 订阅；新条目前置插入列表；"加载更多"按钮按 cursor 翻页；显示 SSE 连接状态指示器

### 评估任务（M13）

- **全局 Provider 注册表** — `llm_providers` 表存储 OpenAI 兼容端点（openai / custom / ollama），API Key 用 AES-256-GCM 加密存储。Key 可解密用于 LLM 调用，但永不返回给前端（仅返回 `****5678` 预览）
- **项目级评估器** — 两种类型：`llm_judge`（调用另一个 LLM 按 judge prompt 评分，正则提取数值并 min-max 归一化到 0-1）和 `numeric_threshold`（对比 trace 指标如 `latency_ms` / `prompt_tokens` / `completion_tokens` 与阈值的大小关系）
- **进程内 Worker** — 单例 `EvalWorker`，`MAX_CONCURRENCY=3`。监听 `eval:job-started` 事件，队列驱动并发执行，支持取消（`cancelledJobs` Set）。服务器重启时 `interruptRunning()` 把未完成的 `running`/`pending` 任务标记为 `interrupted`
- **每条 case 生成 trace** — 每个评估用例在一个 DB 事务中生成完整 trace（`metadata.source='eval'`）+ observation（`type='generation'`），可在 Traces 页面逐条调试 LLM 调用
- **状态机** — Job：`pending → running → completed/failed/cancelled/interrupted`；Item：`pending → running → success/failed`。所有 item 终态后 Job 自动结束并聚合 summary（每个 evaluator 的 avg / passRate / count）
- **SSE 进度流** — `GET /api/stream/eval/:jobId` 推送 `eval:item-completed` 和 `eval:job-completed` 事件；前端详情页实时刷新进度条和 item 列表
- **5 个前端页面** — Provider 管理（创建/编辑/测试连通性/删除）、评估器管理（项目级、类型感知表单）、Job 列表（状态徽章 + 进度）、新建 Job（dataset/prompt 版本/provider/evaluator 下拉）、Job 详情（SSE 实时进度 + summary 表 + item 列表含 trace 跳转）
...
| `ENCRYPTION_KEY` | server | — | **必填（M13）**。AES-256-GCM 密钥，用于加密 Provider API Key。用 `openssl rand -base64 32` 生成（必须解码后恰好 32 字节） |
...
- [x] **M9 — 前端架构**：SSR Cookie 转发、三文件 API 拆分、项目选择器、共享 Nav、Error/Loading 边界
- [x] **M10 — 安全加固**：API Key SHA-256 哈希、IDOR projectId 校验、分层限流
- [x] **M11 — 审计日志**：`onResponse` 全局钩子、`audit_logs` 表、`deriveAction` 纯函数、`/audit` 页面
- [x] **M12 — 实时 SSE + 分页**：EventBus + SSE 三路流（traces/alerts/audit）、cursor 分页
- [x] **M13 — 评估任务**：全局 Provider 注册表（AES-256-GCM 加密）、项目级评估器（llm_judge + numeric_threshold）、进程内 Worker（并发 3 + 崩溃恢复）、每条 case 生成可调试 trace、SSE 实时进度、5 个前端页面
- [ ] **未来**：多租户组织、ClickHouse 迁移、分布式 worker 集群
